...
internal class UpdateValidationBehavior : IPipelineBehavior<UpdatePriceHistory, Result<PriceHistoryModel>>
{
    ...
    public async Task<Result<PriceHistoryModel>> Handle(UpdatePriceHistory request, RequestHandlerDelegate<Result<PriceHistoryModel>> next, CancellationToken cancellationToken)
    {
        try
        {
            var result = Result.BadRequest<PriceHistoryModel>();

            // Ensure the Quantity value is greater than zero
